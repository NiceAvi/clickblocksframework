 ...
 * @final
 */
final class CB implements \ArrayAccess
{
  /**
  ...
  public static function exception($e)
  {
    ...
    $delegateExists = class_exists('ClickBlocks\Core\Delegate', false);
    ...
    {
      if (isset($config['autoload']['callback'])) self::$instance->al('ClickBlocks\Core\Delegate', true);
      else
      {
        $classes = self::$instance->getClassMap();
        if (isset($classes['clickblocks\core\delegate']) && file_exists($classes['clickblocks\core\delegate'])) require_once($classes['clickblocks\core\delegate']);
      }
      $delegateExists = class_exists('ClickBlocks\Core\Delegate', false);
    }
    try
  ...
  public static function analyzeException($e)
  {
    ...
    {
      if (strpos($file, \CB::getRoot()) === 0) $file = substr($file, strlen(\CB::getRoot()) + 1);
   ...
   * @param string $file - file for log.
   * @access public
   * @static
   */
  ...
  {
    if ($file) {
      file_put_contents(self::dir('logs') . '/' . $file, json_encode($data), FILE_APPEND);
    } else {
      $path = self::dir('logs') . '/' . date('Y F');
      if (is_dir($path) || mkdir($path, 0775, true)) {
        file_put_contents($path . '/' . date('d H.i.s#') . microtime(true) . '.log', json_encode($data), FILE_APPEND);
      }
    }
  ...
  /**
   * Initializes the ClickBlocks framework.
  ...
  private function al($class, $auto = true)
  {
    if ($auto && isset($this->config['autoload']['callback']))
    {
      ...
      if (class_exists('ClickBlocks\Core\Delegate', false)) return self::delegate($callback, $class);
  ...
  private function find($class = null, array $options = null)
  {
    ...
    {
      $classmapFile = empty($this->config['autoload']['classmap']) ? null : self::dir($this->config['autoload']['classmap']);
      if (!$classmapFile) throw new \Exception(self::error($this, 'ERR_GENERAL_5'));
      if (file_exists($this->classmap) && (require($this->classmap)) === false)
      {
        ...
        while (($classes = require($classmapFile)) === false && ++$seconds <= $timeout) sleep(1);
        if ($seconds <= $timeout)
        {
    ...
    foreach ($paths as $path => $isRecursion)
    {
      ...
      {
        if ($item == '.' || $item == '..' || $item == '.svn' || $item == '.hg' || $item == '.git' || $item == 'adminer.php') continue;
        ...
        if (is_file($file))
        {
          ...
          for ($i = 0, $max = count($tokens), $namespace = ''; $i < $max; $i++)
          {
            ...
            switch ($token[0])            
            {
              ...
              case T_INTERFACE;
              case T_TRAIT:
                while ($i < $max && is_array($tokens[++$i]) && $tokens[$i][0] == T_WHITESPACE) {}
                $t = $tokens[$i];
   ...
   *
   * @return ClickBlocks\Net\Request
   * @access public
   */
   ...
   *
   * @return ClickBlocks\Net\Response
   * @access public
   */
  ...
  {
    $classmapFile = empty($this->config['autoload']['classmap']) ? null : self::dir($this->config['autoload']['classmap']);
    if ($classmapFile != $this->classmap)
    {
      $this->classmap = $classmapFile;
      $this->classes = file_exists($classmapFile) ? (array)require($classmapFile) : [];
    }
    return $this->classes;
  }
